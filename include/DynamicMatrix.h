...
public:
  ...
  DynamicMatrix& operator=(std::initializer_list<T> mat); // TODO - Fix const bug
...
template <typename T>
DynamicMatrix<T> operator*(IMatrix<T> const& m1, IMatrix<T> const& m2) noexcept(TYPE_CHECKED) {
